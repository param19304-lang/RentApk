...
### Optimization — minification + resource shrinking (now enabled)

`isMinifyEnabled = true` and `isShrinkResources = true` are now on for the
release build type. `app/proguard-rules.pro` has targeted rules based on an
actual risk pass for this codebase — not generic boilerplate:

- **Kept, with a real reason**: `RentReminderWorker` (WorkManager persists a
  worker's class name as a string and can resolve it via reflection on
  process restart, independent of Hilt's own compile-time DI wiring for the
  worker — a genuine, documented gotcha); crash readability attributes
  (`SourceFile`, `LineNumberTable`, annotations) so a minified build's stack
  traces stay locatable.
- **Kept, as cheap insurance rather than a strict requirement**: Room
  entities/DAOs/database. Room's KAPT step resolves entity↔column mapping at
  *compile* time into direct generated code, not runtime reflection, so
  these likely don't need explicit keep rules — but they're ~10 small data
  classes, so keeping them costs essentially nothing.
- **Deliberately NOT duplicated**: Room, Hilt, WorkManager, DataStore, and
  Compose all ship their own `consumer-rules.pro` inside their AARs, merged
  in automatically — Hilt+R8 and Compose+R8 in particular are among the most
  production-tested integrations in the whole Android ecosystem. Re-adding
  rules those libraries already provide would just be noise.

`release.yml` now also runs `bundleRelease` alongside `assembleRelease`
(uploaded as its own artifact) — an `.aab`, not just an `.apk`, is what the
Play Store actually wants for real distribution, since it lets Google
generate per-device-optimized APKs rather than shipping one universal one.
It also uploads `app/build/outputs/mapping/release/mapping.txt` as a build
artifact: **do not lose this file**. Once a release build is minified, class
and method names in any crash log are obfuscated — this mapping file is the
only way to de-obfuscate a stack trace back to your actual source, and it's
specific to the exact build it came from (a mapping.txt from one version
cannot de-obfuscate a crash from a different version).

**Residual risk, stated plainly**: this environment still has no Android SDK
or network access, so I still can't produce and run an actual minified build
myself — same limitation as every build in this project, but it matters more
here specifically, because a minification problem is far more likely to
surface as a **runtime crash on-device** than as a compile error CI would
catch. `assembleRelease` succeeding in CI proves the build compiles and
signs; it does not prove nothing was stripped that's only reached via
reflection at runtime. Before you trust this build: install the signed
release APK on a real device and manually exercise the paths most likely to
expose an R8 issue if my analysis missed something — log in, add a
property/tenant/lease, generate rent, record a payment and share the PDF
receipt, wait for (or trigger) a notification, and try backup/restore. If
anything crashes, send me the crash log (with `mapping.txt` from that exact
build) and I'll fix the specific rule that's missing rather than guess.
